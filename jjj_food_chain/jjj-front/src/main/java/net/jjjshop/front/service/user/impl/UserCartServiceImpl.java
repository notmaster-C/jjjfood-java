...
public class UserCartServiceImpl extends BaseServiceImpl<UserCartMapper, UserCart> implements UserCartService {
    ...
    public Boolean add(User user, CartParam param) {
        ...
        UserCart userCart = this.detail(user.getUserId(), param.getProductId(), param.getProductSkuId());
        if (userCart != null) {
            // 更新
            UserCart updateBean = new UserCart();
     ...
     * @return
     */
    public UserCart detail(Integer userId, Integer productId, Integer productSkuId) {
        return this.getOne(new LambdaQueryWrapper<UserCart>()
                .eq(UserCart::getUserId, userId)
                .eq(UserCart::getProductSkuId, productSkuId)
