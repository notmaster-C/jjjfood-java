      ...
      <div class="table-wrap">
        <el-tabs v-model="activeName" @tab-change="handleClick">
          <el-tab-pane label="全部订单" name="all">
            <template #label>
              <span>全部订单 <el-tag size="mini">{{orderCount.all}}</el-tag></span>
            </template>
          </el-tab-pane>
          <el-tab-pane :label="'未付款'" name="payment">
            <template #label>
              <span>未付款 <el-tag size="mini">{{orderCount.payment}}</el-tag></span>
            </template>
          </el-tab-pane>
          <el-tab-pane :label="'进行中'" name="process">
            <template #label>
              <span>进行中 <el-tag size="mini">{{orderCount.process}}</el-tag></span>
            </template>
          </el-tab-pane>
          <el-tab-pane :label="'已取消'" name="cancel">
            <template #label>
              <span>已取消 <el-tag size="mini">{{orderCount.cancel}}</el-tag></span>
            </template>
          </el-tab-pane>
          <el-tab-pane :label="'已完成'" name="complete">
            <template #label>
              <span>已完成 <el-tag size="mini">{{orderCount.complete}}</el-tag></span>
            </template>
          </el-tab-pane>
        </el-tabs>
        <el-table size="small" :data="tableData.data" :span-method="arraySpanMethod" border style="width: 100%"
              ...
              <template v-else>
                <div class="product-info" v-for="(item, index) in scope.row.product" :key="index">
                  ...
                  <div class="info">
                    ...
                    <div class="orange" v-if="item.refund">{{ item.refund.type }}-{{ item.refund.status }}
                    </div>
                  </div>
                  <div class="d-c-c d-c">
          ...
          <el-table-column prop="deliveryType" label="配送方式">
            <template #default="scope">
              ...
              <span v-if="scope.row.deliveryType == 30">手机号:{{scope.row.user.mobile}}</span>
              </div>
            </template>
          </el-table-column>
          <el-table-column fixed="right" label="操作" width="200">
            <template #default="scope" >
              ...
              <el-button
                v-if="scope.row.orderStatus==10&&scope.row.payStatus==20&&scope.row.refundMoney==0"
                @click="refundClick(scope.row)" type="text" size="small" v-auth="'/takeout/Operate/refund'">退款
              </el-button>
              <el-button
                v-if="scope.row.orderStatus==10&&scope.row.deliveryStatus==10&&scope.row.payStatus==20"
                @click="cancelClick(scope.row)" type="text" size="small" v-auth="'/takeout/operate/orderCancel'">取消
              </el-button>
              <el-button
                v-if="scope.row.orderStatus==10&&scope.row.deliveryType==20&&scope.row.payStatus==20"
                @click="verifyClick(scope.row)" type="text" size="small" v-auth="'/takeout/operate/extract'">核销
              </el-button>
              <el-button
                v-if="scope.row.deliverSource==10&&scope.row.orderStatus==10&&scope.row.deliveryType==10&&scope.row.payStatus==20&&scope.row.deliveryStatus==20"
                @click="verifyClick(scope.row)" type="text" size="small" v-auth="'/takeout/operate/extract'">确认送达
              </el-button>
              <el-button @click="senDada(scope.row)"
                v-if="scope.row.payStatus==20&&scope.row.deliverStatus==0&&scope.row.orderStatus==10&&scope.row.deliveryType==10"
    ...
    methods: {
      ...
      verifyClick(row) {
        ...
        ElMessageBox.confirm('是否确认此操作?', '提示', {
          ...
          OrderApi.takeExtract(extractForm,
              true
            )
            .then(data => {
              self.loading = false;
      ...
      window.location.href = baseUrl + '/api/shop/takeout/operate/export?' + qs.stringify(Params);
    },
      /*打开取消*/
      cancelClick(item) {
