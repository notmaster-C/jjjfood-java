<template>
  ...
  <div class="user">
    ...
    <div class="common-seach-wrap">
      <el-form size="small" :inline="true" :model="searchForm" class="demo-form-inline">
        ...
        <el-form-item label="配送方式">
          <el-select size="small" v-model="searchForm.deliverSource" placeholder="请选择">
            <el-option label="全部" value=""></el-option>
            <el-option v-for="(item, index) in deliverSource" :key="index" :label="item.name" :value="item.value">
            </el-option>
          </el-select>
        </el-form-item>
    ...
    <div class="product-content">
      <div class="table-wrap">
        <el-table size="small" :data="tableData.data" border style="width: 100%" v-loading="loading">
          ...
          <el-table-column prop="orders.payPrice" label="订单金额"></el-table-column>
          <el-table-column prop="price" label="配送费"></el-table-column>
          <el-table-column prop="statusText" label="订单状态"></el-table-column>
          <el-table-column prop="deliverStatusText" label="配送状态"></el-table-column>
          <el-table-column prop="deliverSourceText" label="配送方式"></el-table-column>
          ...
          <el-table-column fixed="right" label="操作" width="120">
            <template #default="scope">
              ...
              <el-button @click="cancelClick(scope.row)"
                v-if="scope.row.deliverSource==20||scope.row.deliverSource==40||scope.row.deliverSource==50&&scope.row.status==10"
                type="text" size="small" v-auth="'/takeout/deliver/cancel'">取消配送
              </el-button>
              <el-button @click="verifyClick(scope.row)" v-if="scope.row.deliverSource==10&&scope.row.status==10"
    ...
    <Cancel v-if="open_edit" :open_edit="open_edit" :deliverId="deliverId" :orderNo="orderNo"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      window.location.href = baseUrl + '/api/shop/takeout/deliver/export?' + qs.stringify(Params);
    },
      /*关闭弹窗*/
      closeDialogFunc(e, f) {
        if (f == 'edit') {
