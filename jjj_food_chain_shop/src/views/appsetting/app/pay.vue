  ...
  -->
    ...
    <el-form-item label="证书类型">
      ...
      <el-form-item label="p12证书" v-if="form.wxPayKind == 2">
        ...
        <div class="gray" v-if="form.p12 != null">已上传p12证书</div>
        <div class="red" v-if="form.p12 == null">未上传p12证书</div>
      </el-form-item>
      ...
      AppSettingApi.payDetail({}, true)
        .then(res => {
          ...
          if(self.app.payTypeJson  && self.app.payTypeJson !== ''){
            self.form.payType = JSON.parse(JSON.stringify(self.app.payTypeJson));
            Object.getOwnPropertyNames(self.form.payType).forEach(function(key){
              for(let i=0;i<self.form.payType[key].payType.length;i++){
                self.form.payType[key].payType[i] = parseInt(self.form.payType[key].payType[i]);
              }
            })
          }else{
