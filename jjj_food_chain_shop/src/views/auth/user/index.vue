<template>
  ...
  <div class="user">
        ...
        <el-table size="small" :data="tableData" border style="width: 100%" v-loading="loading">
          ...
          <el-table-column prop="user_name" label="门店状态">
            <template #default="scope">
              ...
              <span v-if="scope.row.supplier.isDelete==1" class="red">已删除</span>
          ...
          <el-table-column prop="isStatus" label="账号状态" width="80">
            <template #default="scope" >
              <div v-if="scope.row.isSuper < 1||(scope.row.isSuper ==1 && scope.row.userType ==1)">
              <el-switch
          ...
          <el-table-column fixed="right" label="操作" width="120">
            <template #default="scope">
              <el-button v-if="scope.row.isSuper < 1" @click="editClick(scope.row)" type="text" size="small"
                v-auth="'/auth/user/edit'">编辑</el-button>
              <el-button v-if="scope.row.isSuper < 1||(scope.row.isSuper ==1 && scope.row.userType ==1)"
    ...
    <Edit :open="open_edit" :roleList="roleList" :shopUserId="curModel.shopUserId" @close="closeEdit"></Edit>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      console.log('issuper',self.tableData.map(item=>item.isSuper));
