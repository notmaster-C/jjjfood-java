<template>
  ...
  <div class="product">
      ...
      <el-tab-pane label="普通分类" name="first">
        ...
        <div class="product-content">
          <div class="table-wrap">
            <el-table size="small" :data="tableData" row-key="categoryId" default-expand-all
              ...
              <el-table-column prop="" label="图片" width="180">
                <template  #default="scope">
                  <img v-if="scope.row.imageId" v-img-url="scope.row.imagePath" alt="" width="50" />
                </template>
              </el-table-column>
        ...
        <div class="product-content">
          <div class="table-wrap">
            <el-table size="small" :data="tableData" row-key="categoryId" default-expand-all
              ...
              <el-table-column prop="" label="图片" width="180">
                <template  #default="scope">
                  <img v-if="scope.row.imageId" v-img-url="scope.row.imagePath" alt="" :width="50" :height="50" />
                </template>
              </el-table-column>
              <el-table-column prop="sort" label="分类排序"></el-table-column>
    ...
    <!--添加-->
    <Add v-if="open_add" draggable :open_add="open_add" :addform="categoryModel" @closeDialog="closeDialogFunc($event, 'add')">
    </Add>
    <!--修改-->
    <Edit v-if="open_edit" draggable :open_edit="open_edit" :editform="categoryModel"
          ...
          PorductApi.takeCatList({}, true)
            .then(res => {
              ...
              self.categoryModel.catList = self.tableData;
               console.log('status',self.tableData.map(item=>item.status));
            })
            .catch(error => {
              self.loading = false;
            });
