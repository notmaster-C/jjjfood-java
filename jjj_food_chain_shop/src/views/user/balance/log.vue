      ...
      <el-form size="small" :inline="true" :model="formInline" class="demo-form-inline">
        ...
        <el-form-item label="余额变动场景">
          <el-select v-model="formInline.scene" placeholder="请选择">
            ...
            <el-option v-for="(item,index) in Scene" :key="index" :label="item.text" :value="item.value"></el-option>
          </el-select>
        </el-form-item>
      ...
      <div class="table-wrap">
        <el-table size="small" :data="tableData" border style="width: 100%" v-loading="loading">
          ...
          <el-table-column prop="scene.text" label="余额变动场景" width="100">
            <template #default="scope">
              <span v-if="scope.row.scene==10" style="color: #409EFF">{{scope.row.sceneText}}</span>
              <span v-if="scope.row.scene==20" style="color: #67C23A">{{scope.row.sceneText}}</span>
              <span v-if="scope.row.scene==30" style="color: #F56C6C">{{scope.row.sceneText}}</span>
              <span v-if="scope.row.scene==40" style="color: #E6A23C">{{scope.row.sceneText}}</span>
              <span v-if="scope.row.scene==50" style="color: #E63C81">{{scope.row.sceneText}}</span>
            </template>
          </el-table-column>
          ...
          <el-table-column prop="description" label="描述/说明" width="200"></el-table-column>
