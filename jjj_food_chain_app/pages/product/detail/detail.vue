<template>
	<view class="good-detail-modal" :data-theme="theme()" :class="theme() || ''">
		...
		<!-- #endif -->
		<!-- 商品详情模态框 begin -->
		<view class="cover"><image v-if="detail.image" :src="detail.image[0].filePath" class="image" mode="aspectFill"></image></view>
		<view class="bg-white">
			<view class="good_basic">
				...
				<view class="gray6 sell-point" style="line-height: 1.5;">{{ detail.sellingPoint || '' }}</view>
			</view>
		</view>
			...
			<view class="wrapper">
				<view class="properties">
					<view class="property" v-if="detail.specType == 20">
						...
						<view class="values">
							<view class="value" @click="selecedtSpec(value, key)" v-for="(value, key) in detail.skuList" :key="key" :class="{ default: value.checked }">
								{{ value.specName }}
							</view>
						</view>
					</view>
					<view class="property" v-if="detail.productAttr.length > 0" v-for="(item, index) in detail.productAttr" :key="index">
						<view class="title">
						...
						<view class="values">
							<view
								...
								v-if="value != ''"
								v-for="(value, key) in item.attributeValue"
								:key="key"
					...
					<view class="property" v-if="detail.productFeed.length > 0">
						...
						<view class="values">
							<view
								...
								v-for="(item, index) in detail.productFeed"
								:key="index"
...
export default {
	...
	computed: {
		...
		price: function() {
			if(this.discount_price){
				return ((this.discount_price * 1 + this.productPrice * 1) * this.form.showSku.sum).toFixed(2);
			}else{
				return ((this.feedPrice * 1 + this.productPrice * 1) * this.form.showSku.sum).toFixed(2);
			}
		...
		/*判断增加数量*/
		isadd: function() {
			return this.form.showSku.sum >= this.stock || this.form.showSku.sum >= this.form.detail.limit_num;
		},
	...
	methods: {
		...
		selectFeed(item, index) {
			let self = this;
			if (self.form.showSku.feed[index] || self.form.showSku.feed[index] === 0) {
		...
		confirmFunc() {
			...
			if (this.form.showSku.productSkuId[0] == null && this.form.detail.specType == 20) {
				uni.showToast({
					title: '请选择规格',
					icon: 'none',
					duration: 2000
		...
		addCart() {
			...
			if(self.discount_price){
				price = self.discount_price * 1 + self.productPrice * 1;
			}else{
