		...
		<view class="top-tabbar">
			...
			<view class="tc  head_top" :style="topBarHeight() == 0 ? '' : 'height:' + topBarHeight() + 'px;'">
				<view class="d-c-c bg-white">
					<view class="left" :class="orderType == 0 ? 'tab-item-top active' : 'tab-item-top'"
						@click="ordertypeFunc(0)">配送订单</view>
					<view class="right" :class="orderType == 1 ? 'tab-item-top active' : 'tab-item-top'"
						@click="ordertypeFunc(1)">店内订单</view>
				</view>
			</view>
			<view class="d-a-c ww100 mt24" v-if="orderType == 0">
				<view :class="deliveryType == 10 ? 'tab-item active' : 'tab-item'" @click="orderStateFunc(10)">外送订单
				</view>
				<view :class="deliveryType == 20 ? 'tab-item active' : 'tab-item'" @click="orderStateFunc(20)">自提订单
				</view>
			</view>
			<view class="d-a-c ww100 mt24" v-if="orderType == 1">
				<view :class="deliveryType == 40 ? 'tab-item active' : 'tab-item'" @click="orderStateFunc(40)">堂食订单
				</view>
				<view :class="deliveryType == 30 ? 'tab-item active' : 'tab-item'" @click="orderStateFunc(30)">打包订单
				</view>
			</view>
		</view>
		<!--列表-->
		<scroll-view scroll-y="true" class="scroll-Y" :style="'height:' + scrollviewHigh + 'px;'" lower-threshold="50"
			...
			<view class="order-list">
				<view class="item d-b-s" v-for="(item, index) in listData" :key="index"
					@click.stop="gotoOrder(item.orderId)">
					<view class="mr10">
					...
					<view class="flex-1">
						<view class="d-b-c pb14">
							...
							<view class="state">
								<text class="dominant f26">{{ item.deliveryStatusText }}</text>
							</view>
						</view>
						...
						<view class="order-bts">
							<block v-if="item.orderStatus == 10">
								...
								<!-- 订单付款 -->
								<block v-if="item.payStatus == 10"><button class="theme-borderbtn fb"
										@click.stop="onPayOrder(item.orderId)">立即支付</button></block>
								<!-- 订单付款 -->
							</block>
						</view>
					</view>
		...
		methods: {
			...
			ordertypeFunc(e) {
				...
				if (self.orderType != e && e == 0) {
					...
					this.dataType = 0;
					self.getData();
				} else if (self.orderType != e && e == 1) {
					self.page = 1;
					self.loading = true;
					self.listData = [];
					this.orderType = e;
					this.deliveryType = 40;
					this.dataType = 0;
					self.getData();
				}
			},
			...
			addOrder(orderId, storeId) {
				uni.setStorageSync('selectedId', storeId);
				this.gotoPage('pages/product/list/store?orderId=' + orderId);
			},
			/*去评论*/
			gotoEvaluate(e) {
				uni.navigateTo({
					url: '/pages/order/evaluate/evaluate?orderId=' + e
				});
			},
