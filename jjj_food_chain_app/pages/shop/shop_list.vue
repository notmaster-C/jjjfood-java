		...
		<scroll-view scroll-y="true" class="scroll-Y" :style="'height:' + scrollviewHigh + 'px;'" lower-threshold="50"
			...
			<view class="address-list">
				<view class="address pt26 p-0-26 bg-white" @click="onSelectedStore(item.shopSupplierId)"
					:class="isShopid(item.shopSupplierId)?'active':''" v-for="(item,index) in storeList" :key="index">
					<view class="info flex-1">
						<view class="user f34 d-b-c">
							<view class="d-s-c">
								<view
									:class="item.shopSupplierId != selectedId?'select-id':'select-id icon iconfont icon-tijiaochenggong'">
								</view>
